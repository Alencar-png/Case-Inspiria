...
def read_scheduling(scheduling_id: int, repo: SchedulingRepository = Depends()):
    return repo.find_one(scheduling_id)

@scheduling.put("/schedulings/{scheduling_id}", response_model=SchedulingResponse)
def update_scheduling(scheduling_id: int, scheduling_data: SchedulingUpdate, repo: SchedulingRepository = Depends()):
    return repo.update(scheduling_id, scheduling_data)

@scheduling.delete("/schedulings/{scheduling_id}")
def delete_scheduling(scheduling_id: int, repo: SchedulingRepository = Depends()):
